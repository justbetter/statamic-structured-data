...
class BlueprintVariableMapper
{
    public function fieldTypeIsEligible(string $fieldType): bool
    {
        $eligibleFieldTypes = [
            'text',
            'assets',
     ...
     */
    public function normalizeBlueprintItems($items): array
    {
     ...
     * @return array<int, array<string, mixed>>
     */
    public function getImportedFieldsetItems(array $importConfig): array
    {
     ...
     */
    public function setFieldData(array $field, ?string $name = null, ?string $description = null, bool $recursive = true): ?array
    {
     ...
     * @return array<int, array<string, mixed>>
     */
    public function getCollectionVariables(string $collectionHandle, array $field): array
    {
     ...
     * @return array<int, array<string, mixed>>
     */
    public function getGroupVariables(array $field, array $fieldConfig): array
    {
    ...
    /**
     * @param  iterable<int, Blueprint>  $blueprints
     * @return array<int, array<string, mixed>>
     */
    public function mapBlueprintsToVariables(iterable $blueprints): array
    {
        /** @var array<int, array<string, mixed>> $fieldsArray */
        $fieldsArray = collect($blueprints)->filter()->reduce(function (array $carry, $blueprint): array {
            $items = $this->normalizeBlueprintItems($blueprint->fields()->items());

            return array_merge($carry, $items);
        }, []);

        return collect($fieldsArray)
